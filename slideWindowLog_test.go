...
func Test_SlideWindowLog(t *testing.T) {
	...
	rt.RateLimiterInit(5, 100, SlideWindowLogType)
...
func TestSlideWindowLogMultiUser(t *testing.T) {
	wg := sync.WaitGroup{}
	wg.Add(3)
	...
	rt := NewRateLimiter(rds)
	rt.RateLimiterInit(10, 100, SlideWindowLogType)
	defer rt.StopRateLimiter()
